...
export function MainLayout(){
  ...
  return(
    <div className="flex w-full min-h-screen">
      <div className="flex items-center justify-center w-2/5 px-16 bg-bgBlue">
        <Forms />
      </div>
      <div className="flex w-full flex-col px-12 py-16 gap-8">
        <div className="flex justify-between items-end">
          ...
          <span className="text-txtBlackColor font-light text-base">{`Total: ${infoTask.length} ${infoTask.length > 1 ? 'tarefas' : 'tarefa'}`}</span>
        </div>
        {
          infoTask.map((task: DateTask) => (
            <Card
              key={task.id}
              id={task.id}
              title={task.title}
